 ...
 */
import { ViewableBuffer, Geometry, Buffer, TYPES, utils, ObjectRenderer, settings, ENV, State, Shader } from 'pixi.js';
...
class BatchGeometry extends Geometry {
    constructor(attributeRedirects, hasIndex, textureAttribute, texturePerObject) {
        ...
        if (textureAttribute && texturePerObject > 0) {
            this.addAttribute(textureAttribute, attributeBuffer, texturePerObject, true, TYPES.FLOAT);
        }
        if (hasIndex) {
    ...
    static from(options) {
        return class extends (options.BatchRendererClass || BatchRenderer) {
            constructor(renderer) {
                super(renderer, options.attribSet, options.indexCountProperty, options.vertexCountProperty, options.textureProperty, options.texturePerObject, options.inBatchIdAttrib, options.stateFunction || (() => State.for2d()), options.shaderFunction, options.geometryFactory, options.BatchFactoryClass);
            }
        };
    }
}
...
export { AttributeRedirect, Batch, BatchFactory as BatchGenerator, BatchRenderer, BatchRendererPluginFactory, BatchGeometryFactory as GeometryPacker, Redirect, ShaderGenerator };
